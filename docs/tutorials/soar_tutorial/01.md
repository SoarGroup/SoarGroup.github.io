...
1. The application operator fires and creates the jugs on the state and
   the name.
2. The operator proposal retracts because the condition that tests for
...
3. A substate is created (ignore for now) because there are no other
   rules to fire.
