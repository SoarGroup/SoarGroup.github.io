{{tutorial_wip_warning("Soar.Tutorial.Part.1.-.Simple.Agents.pdf")}}
...
![The Soar Debugger](Images/01/image1.png)
...
to use VisualSoar. [VisualSoar][VSIDE] is the Soar development environment and it
...
recommend [VisualSoar][VSIDE] because it has internal support for creating Soar
programs. [VisualSoar][VSIDE] can be found in the same place that the Soar
...
together. The actual grouping of rules in [VisualSoar][VSIDE] is actually
...
tree-like structure of working memory. [VisualSoar][VSIDE] has a structure called
...
window. [VisualSoar][VSIDE] not only lets you visually construct what memory
...
window in [VisualSoar][VSIDE] you will see that the file at the bottom is titled
...
this [VisualSoar][VSIDE] automatically creates a template of it for you (it is
the first rule at the top). It may look a little different because
[VisualSoar][VSIDE] uses a few shortcuts that you will learn later, feel free to
...
[VisualSoar][VSIDE] did) into a single structure where the identifier `(<s>)` is
...
that [VisualSoar][VSIDE] has attempted to write this rule for you also. The
application of initializing the water-jug is not as standard as the
proposal and [VisualSoar][VSIDE] is not able to initialize all of the attributes
...
list of blue warnings. What you just did was make [VisualSoar][VSIDE] check all
...
windows open. You will notice that [VisualSoar][VSIDE] has already included the
...
Now have [VisualSoar][VSIDE] check all productions against the Datamap (using the
...
**VisualSoar** – This rule is an elaboration. [VisualSoar][VSIDE] has a folder
...
name “jug”. (Note: sometimes the display of the Datamap in [VisualSoar][VSIDE] is
...
grouping in [VisualSoar][VSIDE] as an advanced way to comment code: it doesn’t
